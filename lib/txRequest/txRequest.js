...
class TxRequest {
  ...
  async inClaimWindow() {
    const now = await this.now()
    return (
      this.claimWindowStart.isLessThanOrEqualTo(now) &&
      this.claimWindowEnd.isGreaterThan(now)
    )
  }
  ...
  async inFreezePeriod() {
    const now = await this.now()
    return (
      this.claimWindowEnd.isLessThanOrEqualTo(now) &&
      this.freezePeriodEnd.isGreaterThan(now)
    )
  }
  ...
  async inReservedWindow() {
    ...
    return (
      this.windowStart.isLessThanOrEqualTo(now) &&
      this.reservedWindowEnd.isGreaterThan(now)
    )
  }
